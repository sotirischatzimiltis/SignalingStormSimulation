...
runtime, so closed-loop controllers — and a multi-agent system — can intervene.

**Just want to run something?** See [`scripts/README.md`](scripts/README.md)
for a practical runbook (every command, every flag, where results land,
troubleshooting) -- this file covers the *why*, that one covers the *how*.
...
```
SignalingStormSimulation/
├── sim/
│   ├── config.py        # parameters + scenario builders        (see sim/README_config.md)
│   ├── simulator.py     # SimPy discrete-event engine           (see sim/README_simulator.md)
│   ├── metrics.py       # utility u(t) + resilience P           (see sim/README_metrics.md)
│   └── controllers.py   # baseline controllers                  (see sim/README_controllers.md)
├── mcp_server/           # MCP server wrapping the simulator     (see mcp_server/README.md)
├── agents/               # coordinator/tactical/strategic over A2A (see agents/README.md)
├── instrumentation/      # MCP/A2A/LLM call recorder + PostgreSQL access  (see instrumentation/README.md)
├── scripts/
...
│   └── README.md              # the practical runbook (every command, flag, troubleshooting)
├── plots/                     # generated figures (created by plot_compare)
├── Dockerfile, docker-compose.yml   # optional containerized backend     (see scripts/README.md)
├── requirements.txt
└── README.md            # this file
```
...
see [`scripts/README.md`](scripts/README.md) "Database setup"): every run's
structured data (config/status, every MCP/A2A/LLM call, every storm's
evolution record) lives there now, not in JSON files (see
...
python -m scripts.run_agentic --scenario multi_storm # 3 storms of growing intensity, one episode --
                                                      # exercises the evolution loop (see agents/README.md)
python -m scripts.run_agentic --backend docker       # same thing, mcp_server + the 3 agents containerized
                                                      # instead of bare processes (see scripts/README.md)
```
...
overhead report (queried back from PostgreSQL). See
[`mcp_server/README.md`](mcp_server/README.md) and
[`agents/README.md`](agents/README.md) for the architecture and LLM backend
configuration (local Ollama by default; pluggable to Anthropic/OpenAI via API
key).
...
[`agents/README.md`](agents/README.md) for why P can exceed 1.0 (not a bug)
and a real ordering bug this surfaced (the fast Lyapunov lever was briefly
gated behind the slow LLM call before the fix).
...
what the agentic system uses so that agent decision latency is incurred
...
- [x] **MCP server** exposing telemetry (resources) + actuators, the Lyapunov solver, and a
      forecaster (tools) — `mcp_server/`, see [`mcp_server/README.md`](mcp_server/README.md)
...
- [x] **Real decision policies**: pluggable LLM backends (local Ollama by default, Anthropic/OpenAI
      ...
      stub-fallback on any provider failure — see [`agents/README.md`](agents/README.md). Decision
      *quality* is still basic prompting on a small local model; not yet tuned.
...
- [x] **Evolution**: Strategic's per-storm memory feeds back into Tactical within one
      ...
      [`agents/README.md`](agents/README.md). PostgreSQL's `storms` table now holds one row per storm.
- [x] **PostgreSQL persistence**: every run's config/status, MCP/A2A calls, LLM call content, and
      ...
      JSON dumps — see [`instrumentation/README.md`](instrumentation/README.md).
- [x] **Docker Compose backend**: mcp_server + the 3 agents can run as containers
      ...
      — see [`scripts/README.md`](scripts/README.md) "Docker backend". The bare subprocess backend
      (default) is unaffected and needs no Docker at all beyond the now-mandatory Postgres container.
